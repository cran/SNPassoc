...
control0 <- sapply(dimnames(summary(o)$coef)[[1]], FUN=strsplit2, split="[.]")
...
if (o$family$family != "binomial") 
dimnames(r) <- list(nombres, c("freq", "diff", paste(level * 100, "%", sep = ""), "C.I.", "P-val"))
else dimnames(r) <- list(nombres, c("freq", "or", paste(level * 100, "%", sep = ""), "C.I.", "P-val"))
class(r) <- "intervals"
r
}
